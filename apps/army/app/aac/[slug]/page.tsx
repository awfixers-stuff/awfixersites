...
import {
  aacArticles,
  getAdjacentAacArticles,
  getAacArticle,
  getAacContent,
  hasAacContent,
} from "@awfixersites/content/aac";

type AacArticlePageProps = {
  params: Promise<{ slug: string }>;
};

export function generateStaticParams() {
  return aacArticles.map((article) => ({ slug: article.slug }));
}

export async function generateMetadata({ params }: AacArticlePageProps) {
  const { slug } = await params;
  const article = getAacArticle(slug);
  if (!article) return { title: "Article not found" };

  return {
    title: `${article.title} | AAC`,
    description: article.summary,
  };
}

export default async function AacArticlePage({ params }: AacArticlePageProps) {
  const { slug } = await params;
  const article = getAacArticle(slug);
  if (!article || !hasAacContent(slug)) notFound();

  const ArticleContent = getAacContent(slug);
  const { previous, next } = getAdjacentAacArticles(slug);
